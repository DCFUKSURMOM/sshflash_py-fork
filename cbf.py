...
PACKET_SIZE = 16384
MAGIC_NUMBER = bytes.fromhex('f0debc9a')
COMPRESSION_SIG = bytes.fromhex('1F8B0800') #gunzip
CBF_VERSION = 1
BLOCK_SIZE = 0x20000
...
def extract(path):
	...
	fimg.write(image)
	fimg.close()
	print(f'Saved as: {kernel_name}')


def create(mem, opath, ipath, write_file=True):
	try:
...
class packer(object):
	def __init__(self, mem, opath, ipath):
		...
		if mem.lower() == 'superhigh':
			...
			self.error('Memory location must be superhigh, high, or low.')
	...
	def pack_to_buffer(self):
		try:
			...
			padding = pad_len * bytes.fromhex('FF')
			buf += padding
			return buf
		except Exception as e:
			error(e)
